...
$client->lp=1;
	...
	//}
		...
		$client->hse = $database->prep(trim($_POST['hse']));		
        $client->noc = $database->prep(trim($_POST['noc']));
		$client->fid = $database->prep(trim($_POST['idf']));
		$client->comm = $database->prep(trim($_POST['comm']));
		$client->mtype = $database->prep(trim($_POST['mtype']));
		$client->married_no = $database->prep(trim($_POST['mcn']));
		$client->partner_Name = $database->prep(trim($_POST['pn']));
		$client->married_date = $database->prep(trim($_POST['mdd']));
		$client->married_cert = $database->prep(trim($_POST['mc']));
		$client->age = $database->prep(trim($_POST['age']));
...
$client->lp=1;
 ...
 if(User::prevent_double($fullname)>0){
		...
		if ($result) {
			$res = $client->update_marriage(1,$trr);
			$success = "USER SAVED";
			if($_POST['bs']=="Yes"){
	  	...
	  	<div class="row">
		    <br />
             <?php if(isset($_GET['edit'])){?>
			  <h3 align="center">Edit Partner Registration</h3>
                <?php  } else{  ?>
             
			 <h3 align="center">Partner Registration</h3>
            <?php
			}
			?>
		    
		    <div class="col-md-3"></div>
		    <form method="post" enctype="multipart/form-data" action="membership_partner.php" class="col-md-6">
...
<input name="admin" type="hidden" value="<?php #echo 1;  ?>" />-->
            ...
            <?php  } else{  ?>
             ...
             <input type="hidden" class="form-control" name="pn" id="tr" value="<?php echo $members['Full_Name'];  ?>" >
             <input type="hidden" class="form-control" name="mc" id="tr" value="<?php echo $members['Marriage_Certificate'];  ?>" >
             <input type="hidden" class="form-control" name="mcn" id="tr" value="<?php echo $members['marriage_cert_no'];  ?>" >
             <input type="hidden" class="form-control" name="mdd" id="tr" value="<?php echo $members['Marriage_Date'];  ?>" >
