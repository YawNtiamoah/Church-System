...
require 'includes/includes.php'; 
 ...
 $output = '';  
 $sql = "SELECT * FROM membership WHERE Full_Name LIKE '%".$database->prep($_POST["search"])."%' OR First_Name LIKE '%".$database->prep($_POST["search"])."%'  OR Surname LIKE '%".$database->prep($_POST["search"])."%' AND Level_Parent='1'  GROUP BY family_ID"; 
           ...
           $output .= '  
                <tr id="'.$row['ID'].'">  
                    ...
                    $output .= '<td>'.$row["Full_Name"].'</td>
                     ...
                     <td><a class="btn btn-success" href="membership_child.php?idf='.$row["ID"].'">Register </a></td>
                </tr>  
           ';  
