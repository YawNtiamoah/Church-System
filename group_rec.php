...
*/
	...
	//	$client->tr_id = $database->prep($trr);
		$client->gid = $database->prep(trim($_POST['gid']));
...
<div class="row">
		    <br />
			  ...
			  <h3 align="center">Edit Group Registration</h3>
                <?php  } else{  ?>
             
			 <h3 align="center">New Group Registration</h3>
            <?php
			}
			?>
		    ...
		    <form method="post" enctype="multipart/form-data" class="col-md-6">
			  <div class="form-group">
			    <label for="v_num">Group ID</label>
			    <input type="text" class="form-control" value="<?php if(isset($_GET['edit'])){echo trim($member['Group_ID']);} else{echo "RPC/".date("d");}  ?>" id="title" placeholder="Group ID" name="gid" required >
			  </div>
              <div class="form-group">
			    <label for="v_num">Group Name*</label>
			    <input type="text" class="form-control" id="v_num"  value="<?php if(isset($_GET['edit'])){echo trim($member['Group_Name']);} else{echo "";}  ?>" placeholder="Group Name" name="gn" required >
			  </div>
              <div class="form-group">
			    <label for="v_num">Group Description*</label>
			   <textarea class="form-control" id="v_num" placeholder="Description" name="desc" required ><?php if(isset($_GET['edit'])){echo trim($member['Description']);} else{echo "";}  ?></textarea>
			  </div>
              <div class="form-group">
			    <label for="v_num">Date Created</label>
			    <input type="date" class="form-control" value="<?php if(isset($_GET['edit'])){echo trim($member['Date_Commence']);} else{echo "";}  ?>" id="v_num" placeholder="Date Created" name="doe" required >
			  </div>
<!--
              <div class="form-group">
