...
*/
	...
	//	$client->tr_id = $database->prep($trr);
        ...
        $tr=$_POST['tid'];
		$result = $client->update_welfare($tr);
		if ($result) {
			...
			//	redirect_to('membership_baptism.php?baptism=$lb&confirm=$lc&marry=$lm');
				...
				?>
				<script>
			window.location="welfare_list.php";
				</script>
				<?php
				}
				else{
	...
	//}
		...
		$client->amt= $database->prep(trim($_POST['amt']));		
        $client->doe = $database->prep(trim($_POST['doe']));
		 $client->reason = $database->prep(trim($_POST['reason']));
		$client->region=$_SESSION['Region'];
		$client->district=$_SESSION['District'];
		$client->branch=$_SESSION['Branch'];

		$result = $client->new_welfare_benefit();
		if ($result) {
...
<div class="row">
		    <br />
			  ...
			  <h3 align="center">Edit Welfare Benefit Record</h3>
                <?php  } else{  ?>
             
  <h3 align="center">New Welfare Benefits Record</h3>
            <?php
			}
			?>
		    ...
		    <form method="post" enctype="multipart/form-data" class="col-md-6">
			  <div class="form-group">
			    ...
			    <input type="text" class="form-control" value="<?php if($_GET['tr']){echo trim($member['Transaction_ID']);} elseif(isset($_GET['edit'])){echo trim($member['Transaction_ID']);} else{echo "";}  ?>" id="title" placeholder="Transaction ID" name="tid" required >
			  </div>
              <div class="form-group">
			    <label for="v_num">Full Name*</label>
			    <input type="text" class="form-control" id="v_num"  value="<?php if($_GET['tr']){echo trim($member['Full_Name']);} elseif(isset($_GET['edit'])){echo trim($member['Full_Name']);} else{echo "";}  ?>" placeholder="Full Name" name="fn" required >
			  </div>
              <div class="form-group">
			    <label for="v_num">Reason*</label>
			   <textarea class="form-control" id="v_num" placeholder="Reason" name="reason" required ><?php if(isset($_GET['edit'])){echo trim($member['Reason']);} else{echo "";}  ?></textarea>
			  </div>
              <div class="form-group">
