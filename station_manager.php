<?php
    ...
    //station details
	$sta = Station::find_by_id($manager['station_id']);
	...
	if (isset($_POST['btnUpdate'])) {
		...
		$user->station_id = $database->prep(trim($_POST['txtStation']));

		$result = $user->update_station_manager($_GET['id']);
		if ($result) {
			echo "<script type='text/javascript'>
						alert('UPDATE SUCCESSFUL');
						location.assign('station_manager.php?id=".$_GET['id']."');
					</script>";
		}
		else{
			echo "<script type='text/javascript'>
 ...
 $(document).ready(function(){  
      $('#districts').on('change', function() {
        var district = $(this).val();  
           ...
           {  
                $.ajax({  
                     url:"fetch_specified_staions.php",  
                     method:"post",  
	...
	<div class="row">
	  	<div class="row">
		    <br /><h3 align="center">Station Manager Profile</h3>
		    ...
		    <form method="post" class="col-md-6">
			  ...
			  <div class="form-group">
				...
				<div class="form-group">
			    <label for="contact">Select Station*</label>
				  <select class="form-control" name="txtStation" id="stations">
					  <option value="<?php echo @$manager['station_id']; ?>"><?php echo @$station['station_name']; ?></option>
				   </select>
				</div>

			  
			  <button type="submit" class="btn btn-success" name="btnUpdate">Update</button> 
			  <a class="btn btn-danger" href="delete_user.php?id=<?php echo $_GET['id']; ?> &&redirect=manage_station_managers.php">Delete</a>
			</form>
		</div>
	  </div>
