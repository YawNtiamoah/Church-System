	...
	//	$client->tr_id = $database->prep($trr);
		...
		if(isset($_FILES['files']['name'])){
		 ...
		 $client->logo = $database->prep(trim($photo['imageUrl']));
			...
			if($x['extension']=="jpg" || $x['extension']=="JPG" || $x['extension']=="JPEG" || $x['extension']=="PNG" || $x['extension']=="png" || $x['extension']=="gif"){
...
';		}
		...
		$client->file_type = $database->prep(trim($file));
		 ...
		 $client->file_type = $database->prep(trim($_POST['fp']));
		}
		if($_POST['ps']=="Yes"){
$client->lp=1;
}
	...
	if($_POST['bs']=="Yes"){
		...
		$client->del=0;
		$client->reg=$_SESSION['Region'];
		$client->dist=$_SESSION['District'];
		$client->branch=$_SESSION['Branch'];
        $tr=$_POST['tr'];
		$result = $client->update_member($tr);
		if ($result) {
			/*$detail = User::find_by_id($tr);
                $members = $database->fetch_array($detail);
				 if($members['Level_Confirm']==1){
													$mem="MC/".$members['ID'];
													 }
												else{
													 $mem="MNC/".$members['ID'];
													...
													$user= new User;
													$db=$user->memberID($members['Transaction_ID'],$mem);*/
	...
	//}
		...
		$fid=rand(1,10).rand(1,20).rand(2,70).rand(4,13).date("d");
		$client = new User;
		$client->tr_id = $database->prep($trr);
		$client->fname = $database->prep(trim($_POST['fn']));
		$client->fn = $database->prep(trim($_POST['fn'].' '.$_POST['sn'].' '.$_POST['on']));
		...
		$fullname=$database->prep(trim($_POST['fn'].' '.$_POST['sn'].' '.$_POST['on']));
		 ...
		 $client->logo = $database->prep(trim($photo['imageUrl']));
			...
			if($x['extension']=="jpg" || $x['extension']=="JPG" || $x['extension']=="JPEG" || $x['extension']=="PNG" || $x['extension']=="GIF" || $x['extension']=="png" || $x['extension']=="gif"){
			$file='<img class="img-responsive" src="images/'.$photo['imageUrl'].'" width="200" height="200" alt="logo"/>
';		}
			else{
	  	...
	  	<div class="row">
		    <br />
             <?php if(isset($_GET['edit'])){?>
			  <h3 align="center">Edit Membership Registration</h3>
                <?php  } else{  ?>
             
			 <h3 align="center">Membership Registration</h3>
            <?php
			}
			?>
		    
		    <div class="col-md-3"></div>
		    <form method="post" enctype="multipart/form-data" action="membership_rec.php" class="col-md-6">
              ...
              <div class="form-group">
			    ...
			    <input type="text" class="form-control" id="v_num" placeholder="Enter Other Name" name="on" value="<?php if(isset($_GET['edit'])){echo trim($member['Other_Name']);} else{echo "";}  ?>" >
			  </div>
              <div class="form-group">
              ...
              <div class="form-group">
               ...
               <div class="form-group">Marital
                ...
                <select  name="ms"  class="form-control" id="ms" required>
                  <option><?php if(isset($_GET['edit'])){echo trim($member['Marital_Status']);} else{echo "--";}  ?></option>
...
<input name="admin" type="hidden" value="<?php #echo 1;  ?>" />-->
			 ...
			 <?php if(isset($_GET['edit'])){?>
               ...
               <input type="hidden" class="form-control" name="fp" id="tr" value="<?php echo $member['file_path'];  ?>" >
               <input type="hidden" class="form-control" name="logo" id="tr" value="<?php echo $member['logo'];  ?>" >
