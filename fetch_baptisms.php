...
require 'includes/includes.php'; 
 ...
 $output = '';  
 $sql = "SELECT * FROM membership WHERE Full_Name LIKE '%".$database->prep($_POST["search"])."%' OR First_Name LIKE '%".$database->prep($_POST["search"])."%'  OR Surname LIKE '%".$database->prep($_POST["search"])."%' AND Level_Baptism='1' "; 
           ...
           $output .= '  
                <tr id="'.$row['ID'].'">  
                    ...
                    $output .= '<td>'.$row["Full_Name"].'</td>
                     <td>'.$row['DOB'].'</td>
                     <td>'.$row['Gender'].'</td>
                     <td>'.$row['Minister_Name_Baptism'].'</td>
					 <td>'.$row['Venue_Baptism'].'</td>
