...
require 'includes/includes.php'; 
 ...
 $output = '';  
 $sql = "SELECT * FROM membership WHERE Full_Name LIKE '%".$database->prep($_POST["search"])."%' OR First_Name LIKE '%".$database->prep($_POST["search"])."%'  OR Surname LIKE '%".$database->prep($_POST["search"])."%'  "; 
           ...
           $output .= '  
                <tr id="'.$row['ID'].'">  
                    ...
                    $output .= '<td>'.$row["Full_Name"].'</td>
                     ...
                     <td><a class="btn btn-success"  href="group_mem.php?tr='.$row["Transaction_ID"].'&grp='.$_GET["grp"].'&fn='.$row["Full_Name"].'&phone='.$row["Phone"].'">Add</a></td>
                </tr>  
